...
pub enum MaskStrategy {
    ...
    Replace,
    /// AES-256-GCM encryption with a fresh operating-system-generated nonce per value.
    /// Ciphertext is deliberately nondeterministic; existing `ENC[...]` values still decrypt.
    Encrypt {
        /// Encryption key.
        key: String,
    },
...
impl std::fmt::Debug for MaskStrategy {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::PartialMask {
                ...
                .debug_struct("PartialMask")
                .field("reveal_prefix", reveal_prefix)
                .field("reveal_suffix", reveal_suffix)
                .field("mask_char", mask_char)
                .finish(),
            Self::Hash { salt } => f
                .debug_struct("Hash")
...
fn encrypt_mask(finding: &PiiEntity, key: &str) -> Result<String> {
    ...
    OsRng.try_fill_bytes(&mut nonce_bytes).map_err(|_| {
        CloakError::EncryptionError("operating system randomness unavailable".into())
    })?;
...
mod tests {
    ...
    #[test]
    fn test_encrypt_mask_uses_fresh_nonces_for_same_finding() {
        let key = test_key();
        let item = finding(EntityType::Email, 0, 16, "user@example.com");
        let strategy = MaskStrategy::Encrypt { key };
        let mut nonces = std::collections::HashSet::new();
        for _ in 0..64 {
            let encrypted = strategy.try_replacement(&item).unwrap();
            let bytes = BASE64_STANDARD
                .decode(&encrypted[4..encrypted.len() - 1])
                .unwrap();
            assert!(nonces.insert(bytes[..NONCE_LENGTH].to_vec()));
            assert_eq!(
    ...
    fn test_legacy_ciphertext_still_decrypts() {
        // Generated by cloakrs-core 0.3.1 on a 64-bit platform before random nonces.
        let encrypted = "ENC[KSRwf86/9WMtKf0ySruNd4emDDyg+cW4FDzotEidbWHwZaayn+wM2/UHktQ=]";
        assert_eq!(
            decrypt_masked_value(encrypted, &test_key()).unwrap(),
            "user@example.com"
        );
    ...
    #[test]
    fn test_strategy_debug_redacts_key_and_salt() {
        let key = test_key();
        assert!(!format!("{:?}", MaskStrategy::Encrypt { key: key.clone() }).contains(&key));
        assert!(!format!(
            "{:?}",
            MaskStrategy::Hash {
                salt: Some("private-salt".into())
            }
        )
        .contains("private-salt"));
    }
    ...
    fn test_decrypt_masked_value_tampered_ciphertext_fails() {
        ...
        let mut bytes = BASE64_STANDARD
            .decode(&encrypted[4..encrypted.len() - 1])
            .unwrap();
        bytes[NONCE_LENGTH] ^= 1;
        let tampered = format!("ENC[{}]", BASE64_STANDARD.encode(bytes));
